...
たとえば、基準となる原点 \(O\) から、ある点 \(P\) へ向かう矢印を考える。この矢印は、原点 \(O\) から見て点 \(P\) がどこにあるかを表している。

この矢印の長さと向きを変えずに平行移動し、始点を別の点 \(O'\) に移すと、終点も別の点 \(P'\) に移る。移動後の矢印は、もとの矢印と同じ大きさと向きを持つが、原点 \(O\) から見た点 \(P\) の位置を表すものではない。
...
ここまで、ベクトルを矢印と成分で表す方法を学んだ。次に、ベクトルを使ってどのような計算ができるかを考える。
...
加法とスカラー倍は、ベクトルを組み合わせたり、大きさや向きを変えたりする操作である。内積と外積は、2つのベクトルの大きさや向きの関係を調べる操作である。
...
蝶番から押す場所へ向かう方向と、ドアを押す方向に注目する。

- ドアに沿って蝶番の方向へ押しても、ドアは回転しない。
- ドアを斜めに押すと、押す力の一部がドアを回転させる方向に働く。
- ドアに対して垂直に押すと、押す力はドアを回転させる方向に最も大きく働く。
- 押す向きを反対にすると、ドアが回転する向きも反対になる。

外積は、このような回転の大きさや向きを表すときに使われる。
...
大きさだけで表される量をスカラーといい、大きさと向きを合わせて持つ量をベクトルという。

ベクトルは矢印によって表される。矢印の長さがベクトルの大きさを表し、矢印の向きがベクトルの向きを表す。

自由ベクトルは、大きさと向きを変えなければ平行移動しても同じものとして扱える。一方、束縛ベクトルでは、始点の位置にも意味がある。
...
次章では、1本のベクトルを方向ごとの成分に分ける方法を考える。さらに、ベクトルの式を成分ごとの数の式として読み取れる理由を、単位ベクトルと基底を使って明らかにする。
